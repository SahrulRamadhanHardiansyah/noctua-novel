    ...
    <div className="bg-black text-white min-h-screen">
      <section className="relative pt-40 pb-16">
        <div className="absolute inset-0">
          {novel.image_url && <Image src={`/api/proxy-image?url=${encodeURIComponent(novel.image_url)}`} alt={`${novel.title} background`} fill unoptimized={true} className="object-cover opacity-50 blur-2xl" />}
          <div className="absolute inset-0" />
        </div>
        ...
        <div className="relative container mx-auto px-6 md:px-16 lg:px-36 flex flex-col md:flex-row gap-8 md:gap-12 items-center md:items-start">
          <div className="w-48 md:w-1/4 flex-shrink-0">
            {novel.image_url ? (
              ...
              <div className="rounded-lg w-full aspect-[3/4] bg-gray-800" />
            )}
          </div>
